...
@Composable
fun EditExerciseScreen(treningViewModel: TreningViewModel,
                      onCLose : () -> Unit )
{
    if( treningViewModel.editedExercise == null ) {
        ...
        val editedExercise = treningViewModel.editedExercise

        var nazwa by remember { mutableStateOf(editedExercise!!.nazwa) }
        var opis by remember { mutableStateOf(editedExercise!!.opis) }
        var spalanie by remember { mutableStateOf(editedExercise!!.met) }
        ...
        val selectedGrupyMiesniowe = remember {
            mutableStateListOf<GrupaMiesniowa>()
                .apply { addAll(editedExercise!!.grupaMiesniowas) } }
        ...
        Column (Modifier
            ...
            Column(Modifier.weight(5f)) {
                NavigationButtonsRetAdd(
                    ...
                    onAdd = {
                        scope.launch {
                            treningViewModel.updateExercise(
                                Cwiczenie(
                                    id = editedExercise!!.id,
                                    nazwa  = nazwa,
                                    opis = opis,
                                    grupaMiesniowas= selectedGrupyMiesniowe,
                                    met = spalanie
                                )
                            )
